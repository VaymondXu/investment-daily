...
（逐条列出，每条格式：`日期时间 [国家] 事件名（前值: X，预期: Y），[结合当前市场环境推导该数据对哪些资产/板块的潜在影响，1句]`）
"""
...
def main():
    ...
    print(f"  2/6 LLM 解读候选池（全局视角，共 {len(polymarket_candidates)} 条 + 极端共识 {len(consensus_candidates)} 条）...")
    polymarket_interpretation = interpret_polymarket(polymarket_candidates + consensus_candidates)

    print("  3/6 LLM 筛选投资相关事件 + 翻译（双维度：24h成交 / 累计投注各 top 5 + 共识信号）...")
