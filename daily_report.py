...
ASSET_NEWS_QUERIES = {
    ...
    "纳斯达克":   "Nasdaq 100 Nasdaq composite index equities stocks performance today",
    "上证指数":   "Shanghai Composite A-share market news today",
    "恒生指数":   "Hang Seng Index HSI performance market movement today",
...
selected 数组长度不超过 {top_n}，按投资相关性从高到低排列。"""
    ...
    if last_exc is not None:
        print(f"[LLM筛选] 筛选+翻译失败，回退前{top_n}条: {last_exc}")
...
def fetch_news() -> dict:
    ...
    macro_queries = {
        ...
        "中国市场":   "China macro economy PBOC policy A-shares capital flow PMI Hang Seng index",
        "美股":       "US stock market S&P500 Nasdaq today",
        "加密货币":   "Bitcoin crypto market today",
    }

    def _search_macro(section, query):
        try:
            ...
            print(f"[Tavily] 宏观 {section} 失败: {exc}")
            return section, {"answer": "", "snippets": []}

    def _search_asset(asset_name, query):
        try:
            res = client.search(query=query, search_depth="basic",
            ...
            print(f"[Tavily] 资产 {asset_name} 失败: {exc}")
            return asset_name, {"answer": "", "snippets": []}

    # Layer A + B 全部并发，15 个请求同时发出
    macro = {}
    per_asset = {}
    with ThreadPoolExecutor(max_workers=15) as pool:
        macro_futures   = {pool.submit(_search_macro, s, q): s for s, q in macro_queries.items()}
        asset_futures   = {pool.submit(_search_asset, n, q): n for n, q in ASSET_NEWS_QUERIES.items()}
        all_futures     = {**macro_futures, **asset_futures}
        for future in as_completed(all_futures):
            key, data = future.result()
            if key in macro_queries:
                macro[key] = data
            else:
                per_asset[key] = data

    # 保证 macro 顺序与 macro_queries 一致（as_completed 顺序不定）
    macro     = {k: macro.get(k, {"answer": "", "snippets": []})     for k in macro_queries}
    per_asset = {k: per_asset.get(k, {"answer": "", "snippets": []}) for k in ASSET_NEWS_QUERIES}
...
def build_data_block(polymarket, assets, news,
                     is_market_closed: bool = False,
                     market_data_date=None) -> str:
    """拼接结构化数据文本，供 LLM 参考"""
    lines = [f"报告生成日期：{TODAY}", ""]
    if is_market_closed:
        data_date_desc = market_data_date or "上一交易日"
        lines += [
            f"⚠️ 【休市提示】当前市场处于休市状态（可能为周末、节假日或周一早盘前）。",
...
6. 大类资产"驱动因素"列提取规则：**以"资产专属新闻"的摘要（answer）字段为第一判据**——只要摘要字段有实质内容，即视为有效来源并直接提取关键词（≤20字），不需要也不应被 snippets 中无关内容影响；仅当摘要字段标注"Tavily无返回"或确实为空时，才升级为**从宏观舆情板块推断**，并加"〔宏观〕"标注；不得借用其他资产的专属新闻；实在无任何信息支撑则填"—"，不得编造；⚠️ 个股/单一公司的财报、IPO、并购、评级等微观事件不构成指数（标普500/纳斯达克/上证/恒生）的有效驱动因素，遇到此类内容应忽略并填"—"
...
10. ⚠️ Polymarket 逻辑推演规范：分析概率变化时必须严格遵守形式逻辑，不得因否定词产生逻辑绕圈。规则：若带有"结束/停火/退出/和解/降级"等缓和字眼的事件概率**下降**，意味着冲突/危机继续风险**上升**，与避险资产（原油、黄金、美元）上涨属于同向逻辑，切勿误判为背离；反之，缓和词事件概率**上升**意味着风险下降，此时避险资产若仍在涨才是真正的背离。
...
def generate_report(data_block: str, is_market_closed: bool = False) -> str:
    """调用 DeepSeek API 生成报告（失败自动重试一次）"""
    client = OpenAI(
        api_key=DEEPSEEK_API_KEY,
        base_url="https://api.deepseek.com",
    )
    report_type = "宏观复盘 (Market Review)" if is_market_closed else "投资日报 (Daily Update)"
...
def main():
    ...
    # 若市场最新数据日期不是今天，说明当前处于休市状态（周末/节假日/周一早盘前）
    ...
    mode_tag = f"复盘模式（数据截至 {latest_data_date}）" if is_market_closed else "实时日报模式"
    ...
    print("  5/5 生成报告...")
    data_block = build_data_block(polymarket, assets, news, is_market_closed=is_market_closed,
                                  market_data_date=latest_data_date)
    report     = generate_report(data_block, is_market_closed=is_market_closed)
