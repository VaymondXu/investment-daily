...
def filter_and_translate_polymarket(items: list[dict], top_n: int = 5) -> tuple:
    """LLM 从候选事件中按双维度各选 top_n 条投资相关事件并翻译为中文。
    返回 (list_24h, list_total)：
    ...
    client = OpenAI(api_key=DEEPSEEK_API_KEY, base_url="https://api.deepseek.com")
    prompt = f"""以下是 Polymarket 预测市场的候选事件列表（JSON 数组，含编号、英文标题、YES概率、24h变化、24h成交量、累计成交量）。
...
请以 JSON 对象返回，格式：
{{
  "by_volume_24h":    [{{"id": 0, "zh": "中文标题"}}, ...],
  "by_volume_total":  [{{"id": 3, "zh": "中文标题"}}, ...]
}}
...
by_volume_24h 按 volume_24h 从大到小排列，by_volume_total 按 volume_total 从大到小排列，每组不超过 {top_n} 条。"""
    ...
    def _build_list(selected_list):
        """将 LLM 返回的 [{id, zh}] 转为带中文标题的 item 列表"""
        out = []
        seen_ids = set()
...
def build_data_block(polymarket_24h, polymarket_total, assets, news,
                     ...
                     polymarket_interpretation: str = "") -> str:
    """拼接结构化数据文本，供 LLM 参考"""
    ...
    lines.append(f"[展示条目：24h活跃 {len(polymarket_24h)} 条 + 长期关注 {len(polymarket_total)} 条，共 {total_shown} 个唯一事件]")
    ...
    for m in polymarket_total:
        lines.append(
            f"- {m['question_zh']} | YES: {m['yes']} | 24h变化: {m['chg_24h']} | 累计投注: {m['volume_total']}"
        )
...
7. Polymarket 分"24h活跃"和"长期关注"两个表格，必须分别完整列出数据块对应分组中的每一条，不得省略；大类资产表格同样必须完整列出所有条目；分析解读只在表格下方的文字中进行
...
|------|---------|---------|-----------|
（**必须完整列出数据块"[24h活跃]"中的所有条目，一条不少，顺序保持原样**；24h变化按数据原样填写，无数据填"--"）

### 长期关注

| 事件 | YES概率 | 24h变化 | 累计投注 |
|------|---------|---------|----------|
（**必须完整列出数据块"[长期关注]"中的所有条目，一条不少，顺序保持原样**；24h变化按数据原样填写，无数据填"--"）

**解读**：[2-3句。数据块中若有"候选池整体解读"，以其为基础精炼输出，不必另起炉灶；优先分析24h变化>5pp的事件，深挖其背后叙事；概率已极高（>90%）或极低（<10%）且变化<2pp的事件无需单独展开，一笔带过即可；多个事件同向时用"共同指向"，真正反向才用"但/矛盾"]
...
def main():
    ...
    print("  1/6 拉取 Polymarket 候选事件...")
    polymarket_candidates = fetch_polymarket()

    print(f"  2/6 LLM 解读候选池（全局视角，共 {len(polymarket_candidates)} 条）...")
    polymarket_interpretation = interpret_polymarket(polymarket_candidates)

    print("  3/6 LLM 筛选投资相关事件 + 翻译（双维度：24h成交 / 累计投注各 top 5）...")
    polymarket_24h, polymarket_total = filter_and_translate_polymarket(polymarket_candidates)
    ...
    print("  5/6 搜索新闻舆情（宏观 + 分资产定向）...")
    news = fetch_news()

    print(f"  6/6 生成报告...（Polymarket：24h活跃 {len(polymarket_24h)} 条，长期关注 {len(polymarket_total)} 条）")
